...
var (
	...
	// userIpRequestHeaders overrides .auth.allowedIpAddress.userIpRequestHeaders from site config.
	//
	// Use case:
	// 	- Cloud needs to manager this header to ensure the user ip is reading from a trusted header.
	userIpRequestHeaders = func() []string {
		if v := os.Getenv("AUTH_ALLOWED_IP_ADDRESS_USER_IP_REQUEST_HEADERS"); v != "" {
...
func buildConfigFromSiteConfig(cfg schema.AuthAllowedIpAddress) *config {
	...
	if len(userIpRequestHeaders) > 0 {
		userHeaders = userIpRequestHeaders
	}

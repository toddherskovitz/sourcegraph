...
func Test_getConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "override user headers from env var",
			mock: &schema.AuthAllowedIpAddress{
				Enabled:              true,
				UserIpRequestHeaders: []string{"x-forwarded-for"},
			},
			want: true,
			before: func() {
				userIpRequestHeaders = []string{"CF-Connecting-IP"}
			},
			after: func() {
				userIpRequestHeaders = nil
			},
		},
	}
	for _, tc := range tests {
